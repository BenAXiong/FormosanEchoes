...
export default function DemoPage({ songs, artists }: Props) {
  ...
  const { playingTrack, playTrack, setQueue, isFavorite, playlists } = usePlayer();

  useEffect(() => { setMounted(true); }, []);
  useEffect(() => { if (window.innerWidth < 1024) setCompact(true); }, []);
  ...
  const results = useMemo(() => {
    ...
    let filtered = filterSongs(searched, filters);
